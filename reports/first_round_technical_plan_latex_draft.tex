...
\begin{center}
{\Large \bfseries 제24회 한국로봇항공기경연대회 중급부문}\\[1.5mm]
{\large \bfseries GPS 비의존 교차점 스프린트 드론 기술개발계획서}\\[3mm]
\begin{tabular}{rlrl}
\textbf{소속} & \placeholder{제출 전 입력} &
\textbf{참가팀명} & \placeholder{제출 전 입력}\\
\textbf{지도교수} & \placeholder{제출 전 입력} &
\textbf{대표자} & \placeholder{제출 전 입력}\\
\textbf{제출일} & 2026년 \placeholder{월 일} &
\textbf{대표자 외 인원 수} & \placeholder{입력}\\
\end{tabular}
\end{center}
...
\begin{abstract}
본 계획서는 실내 GPS 비허용 조건에서 3m 격자 교차점과 하향 카메라를 이용해 위치를 보정하는 자율비행 시스템을 제안한다. 핵심 가설은 격자선을 연속적으로 추종하는 저속 라인 PID 대신, 교차점을 위치 보정 기준으로 삼아 고속 이동하고 ArUco 마커 후보가 확인될 때만 감속ㆍ정렬ㆍ3초 호버링을 수행하는 것이다. 현재 구현 범위는 ROS2 C++ 기반 \code{grid\_detector}, \code{aruco\_tracker}, \code{visual\_odometry}, \code{sprint\_planner}, \code{mission\_manager}, \code{px4\_offboard\_control}, \code{vertiport\_detector} 패키지, 10m/s 목표 headless 전체 미션 기록, Gazebo/PX4 SITL smoke check이다. 본문 수치는 headless 완주 기록, PX4 smoke check, 경로 비용 모델을 구분하여 표기한다.
\end{abstract}

\textbf{Keywords:} GPS-denied UAV, optical flow, grid localization, ArUco, PX4 Offboard, ROS2, Gazebo SITL
...
운용 단계는 Fig. 2와 같이 Phase 0--3으로 구성한다. Phase 1에서는 교차점-to-교차점 S자 탐색으로 규정상 구획선 이동 요구를 만족시키고, 마커 후보가 확정될 때만 접근ㆍ흔들림 억제ㆍ3초 호버링을 수행한다. Phase 2에서는 구조 경로의 구획선 추종 미검증 조건을 활용하여 저장 좌표 사이를 직선으로 이동한다.
...
\end{equation}
경로 후보 $\rho$의 비용은 이동 시간뿐 아니라 회전, 정지, 마커 확인, 안정화, 위험도를 함께 반영한다.
\begin{equation}
J(\rho)=
w_t(T_{\mathrm{motion}}+T_{\mathrm{turn}}+T_{\mathrm{hover}}+T_{\mathrm{stabilize}})
w_nN_{\mathrm{stop}}
w_rR
-w_cC_{\mathrm{coverage}}.
\end{equation}
...
Phase 1 탐색 경로는 9 $\times$ 6 = 54개 교차점을 지나는 boustrophedon(S자) 순서다. 모든 교차점에서 정지하지 않고, 교차점은 위치 보정 체크포인트로 사용한다. 단, ArUco 후보가 확인되어 confidence 조건을 만족하면 접근 모드로 전환하고, 흔들림 억제 후 3초 호버링을 수행하여 규정상 첫 인식 확인 조건을 만족시킨다.

\begin{figure}[H]
\centering
\includegraphics[width=0.80\linewidth]{fig_05_gazebo_search_path.png}
\caption{Phase 1 교차점-to-교차점 S자 탐색 경로. seed 310 마커 배치와 54개 교차점 후보를 함께 표시한다.}
\end{figure}

Phase 2 구조 경로는 저장된 마커 좌표를 ID 내림차순으로 정렬하여 직선으로 연결한다. Fig. 6에서 구조 구간이 격자선을 따라 직각으로 이동하지 않는 이유는, 이 단계의 1차 목적을 선 추종 정확도보다 \textbf{구조 대상의 신속한 재방문}으로 두기 때문이다. 즉, 구조 순서와 마커 통과 조건을 만족하는 범위 안에서 경로 길이와 감가속 시간을 최소화하고, 직각 격자 경로가 만드는 불필요한 회전ㆍ정지 비용을 제거한다. 이는 정확도 점수를 완전히 포기한다는 의미가 아니라, 구조 시간 단축으로 얻는 임무 수행 시간 이득과 위치 정확도 감점 위험을 하나의 비용함수에서 비교하는 선택이다.
\begin{equation}
\rho^\star_{\mathrm{rescue}}
=\arg\min_{\rho}
\left[
T_{\mathrm{travel}}(\rho)
+T_{\mathrm{brake}}(\rho)
+T_{\mathrm{hover}}(\rho)
+\lambda_{\mathrm{acc}}P_{\mathrm{pos}}(\rho)
\right],
\end{equation}
여기서 $P_{\mathrm{pos}}$는 저장 좌표 접근 오차, 재검출 실패, 중심 정렬 오차에 따른 정확도 penalty이다. 현재 headless 기록과 비용 모델은 구조/귀환 스프린트 속도를 10m/s로 두었지만, 향후 목표는 동일 구조 경로를 20m/s급으로 확장하는 것이다. 다만 속도 $v$가 증가하면 마커가 카메라 시야의 유효 검출 폭 $L_{\mathrm{vis}}$ 안에 머무는 프레임 수가
\begin{equation}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{fig_06_gazebo_mission_path.png}
\caption{전체 미션 경로와 구조 방문 순서. 구조 단계는 직각 격자 추종보다 구조 시간 단축을 우선하여 ID 역순 마커를 직선 스프린트로 연결하고, 종단부에서 재검출ㆍ정렬ㆍ호버링으로 정확도 위험을 보상한다.}
\end{figure}
...
\item \textbf{물리 기반 비용 함수:} 거리보다 실제 시간에 가까운 가속/감속/회전/정지/호버 비용을 사용한다.
...
\centering
\small
\begin{tabularx}{0.92\linewidth}{p{28mm}Y}
\toprule
\textbf{항목} & \textbf{내용}\\
\midrule
성명 & \placeholder{지도교수명 입력}\\
소속 & \placeholder{학과/연구실 입력}\\
역할 & 기술 방향 검토, 안전관리, 시험계획 검토, 대회 규정 해석 자문\\
\bottomrule
\end{tabularx}
\end{table}
...
\centering
\small
\begin{tabularx}{0.96\linewidth}{p{28mm}p{34mm}Y}
\toprule
\textbf{이름} & \textbf{담당} & \textbf{역할}\\
\midrule
\placeholder{팀원 1} & Mission/Planning & 상태 기계, rescue order, sprint cost model, sweep 분석\\
\placeholder{팀원 2} & Perception & Grid detector, ArUco tracker, vertiport detector, 영상 시험\\
\placeholder{팀원 3} & Control/SITL & PX4 Offboard, mode별 envelope, Gazebo/PX4 통합\\
\placeholder{팀원 4} & Integration/Report & launch/config, rosbag 기록, figure 생성, 기술 문서화\\
\bottomrule
\end{tabularx}
\end{table}
...
최종 제출 전 보강해야 할 항목은 실제 부품 모델명, 공식 ArUco dictionary, 버티포트 외형, 시간 측정 시작/종료 기준, 고속 ArUco 실험 결과, PX4/Gazebo 전체 미션 rosbag이다. 본 초안은 현재 저장소에 존재하는 구현과 데이터에 맞춰 작성했으며, headless 완주 기록과 mission-cost sweep을 실기체 성능으로 과장하지 않는다.

\begin{table}[H]
\centering
\small
\caption{보고서 수치의 출처와 해석}
\begin{tabularx}{\linewidth}{p{60mm}Y}
\toprule
\textbf{자료} & \textbf{보고서에서의 사용 방식}\\
\midrule
\code{docs/contest\_rule.md} & 경기장 크기, 격자, 마커, 채점 항목, 3초 호버링, 구조 역순 규칙의 기준\\
\code{docs/development\_status.md} & 구현 완료 패키지와 로컬 검증 현황\\
\code{reports/marker\_layout\_sweep.csv} & 600개 랜덤 배치 mission-cost sweep 수치\\
\code{reports/phase1\_speed\_sweep.csv} & Phase 1 속도 5/7/10m/s trade-off 수치\\
\code{bags/headless\_20260517\_204437/run} & 10m/s 목표 headless 전체 미션 텔레메트리와 Fig. 8 생성 근거\\
\code{reports/gazebo\_run\_20260517\_054903/bag} & PX4/Gazebo smoke check 보조 근거\\
\code{src/*/test} & 핵심 알고리즘 단위시험 근거\\
\bottomrule
\end{tabularx}
\end{table}
